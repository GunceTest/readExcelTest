...
//	}
	...
	protected String removeDotAndZeros(String txt) {
		txt = txt.replace(".", "");
		txt = txt.replace(",00", "");
		return txt;
		
	}
	protected String removeDotAndZerosandToplam(String txt) {
		txt = txt.replace(".", "");
		txt = txt.replace(",00", "");
		txt = txt.replace("Toplam ", "");
		return txt;
		
	}
	protected String removeSubTextFromText(String txt, String txtToBeRemoved) {
		txt = txt.replace(txtToBeRemoved, "");
		return txt;
...
//		}
	...
	protected String getTextContent(By elementBy) {
		waitAllRequest();
		return driver.findElement(elementBy).getAttribute("textContent");
		}
	
	protected String getTextContentJS(String element_attribute, int index) {
		waitAllRequest();
		sleep(1);
	return	(String) js.executeScript("return document.querySelectorAll('" + element_attribute + "')[" + index + "].textContent;");
